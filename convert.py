# convert.py
"""
Chuẩn bị asset HLS local từ các file mp4 đã tải về, theo đúng định dạng client Android
(ZVideoPlus) đang mong đợi - xem des.md.

Với mỗi mp4 trong INPUT_DIR, script tạo ra:
    public/hls/<video_id>/master.m3u8            -> master playlist MULTIVARIANT (>= 3 rendition)
    public/hls/<video_id>/<tier>/index.m3u8      -> media playlist từng rendition (VOD + ENDLIST)
    public/hls/<video_id>/<tier>/<video_id>.mp4dv-> 1 file fMP4 duy nhất / rendition (byte-range)
    public/thumbs/<video_id>.jpg                 -> frame đầu tiên của video
...
Vì sao phải viết lại URI trong master:
    ffmpeg ghi URI tương đối ("pg_5/index.m3u8"). Master được phục vụ tại
    /video/upload/sp_auto/<video_id>.m3u8 nên URI đó sẽ resolve thành
    /video/upload/sp_auto/pg_5/index.m3u8 - KHÔNG có video_id, mọi video đâm vào cùng URL.
    Nên master được viết lại thành absolute path có kèm video_id (des.md §1 cho phép
    absolute path vì client dùng UriUtil.resolve).

Cài đặt trước khi chạy:
    ffmpeg + ffprobe phải có trong PATH.

Cách chạy:
    python convert.py [thư_mục_input] [--force]
    (mặc định: downloads; --force để tạo lại cả video đã convert)
"""
...
# Tiền tố URL mà main.py phục vụ. Phải khớp với route trong main.py.
URL_PREFIX = "/video/upload/sp_auto"
...
def rewrite_master(master: Path, video_id: str) -> None:
    """
    Đổi URI variant từ tương đối ("pg_5/index.m3u8") sang absolute path kèm video_id
    ("/video/upload/sp_auto/pg_5/<video_id>.m3u8") - xem docstring đầu file.
    """
    ...
    for line in lines:
        ...
        if stripped.endswith("/index.m3u8"):
            tier = stripped.split("/")[0]
            out.append(f"{URL_PREFIX}/{tier}/{video_id}.m3u8")
        elif stripped:
            out.append(line)
    master.write_text("\n".join(out) + "\n", encoding="utf-8")
...
def convert_one(src: Path, force: bool) -> tuple[bool, str]:
    ...
    if master.exists():
        ...
        rewrite_master(master, video_id)

    # ---- Thumbnail: frame đầu tiên ----
    if thumb.exists():
        notes.append("thumb đã có")
    else:
        ok, err = run([
            "ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
            "-i", str(src), "-frames:v", "1", "-q:v", "2", "-an", str(thumb),
        ])
        if not ok:
